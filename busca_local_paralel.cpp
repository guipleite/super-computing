...
int main(){
    ...
    std::vector<int> sat(iter,-1) ;
    std::vector<std::vector<int>> sat_v(iter);
   
    bool f= true;

    #pragma omp parallel for //reduction(max :satisfacao_atual)
    for(int k=0;k<iter;k++){
      int choice;
      int  satisfacao_atual;

      std::mt19937 eng(seed+k+26); // seed the generator
      ...
      sat[k] = std::get<1>(buff);
      sat_v[k] = std::get<0>(buff);
    ...
    for(int i = 0;i<iter;i++){
      if(sat[i]>gt){
        ...
        //Para rodar o Jupyter comentar a parte a seguir:
        std::cerr << "Iteracao " << gt;
        for (auto i=sat_v[in].begin(); i!=sat_v[in].end(); i++){
            std::cerr << " " << *i ;
        }
        std::cerr << "\n";
